...
SystemInfoCollector::collector_function() {
    ...
    while (running_) {
        /* collect metrics */
        knowhere_keeping_alive_counter_.Increment(1);

        // cpu_utilization_ratio range: 0~25600%
        knowhere_cpu_util_ratio_gauge_.Set(cpu_utilization_ratio());

        // cpu_temperature range: -40°C~120°C
        knowhere_cpu_temp_gauge_.Set(cpu_temperature_celsius());

        // mem_usage range: 0~1TB
        knowhere_mem_usage_gauge_.Set(mem_usage());

        // mem_available range: 0~1TB
        knowhere_mem_avail_gauge_.Set(mem_available());

        // network_receive_bytes_total range: 0~1GB
        knowhere_network_rx_bytes_total_gauge_.Set(network_receive_total());

        // network_transport_bytes_total range: 0~1GB
        knowhere_network_tx_bytes_total_gauge_.Set(network_transport_total());
